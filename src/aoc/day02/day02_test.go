...
var testCases = []testCase{
	testCase{[]int{1, 0, 0, 0, 99}, 2},
	testCase{[]int{2, 3, 0, 3, 99}, 2},
	testCase{[]int{1, 1, 1, 4, 99, 5, 6, 0, 99}, 30},
	testCase{[]int{1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50}, 3500},
...
func TestIntcode(t *testing.T) {
	for _, tc := range testCases {
		got := NewIntcode(&tc.input).Execute()

		if got != tc.expected {
			t.Errorf("NewIntcode(%d).Execute() = %d, want %d", tc.input, got, tc.expected)
		}
	}
}
